...
export const configSchema = {
  selectedInterpreter: {
    ...
    description:
      'Full path to the interpreter chosen through **Autocomplete Python Pulsar: Select Interpreter** or the status bar. Takes priority over everything below. Clear it to go back to automatic discovery.'
  },
  showDescriptions: {
    type: 'boolean',
    default: true,
  ...
  useSnippets: {
    ...
    description:
      'Fill in function arguments after typing the opening parenthesis. `required` inserts only parameters without a default. Use the `autocomplete-python-pulsar:complete-arguments` command to trigger this manually. Requires the bundled `snippets` package.'
  },
  pythonPaths: {
    type: 'string',
    default: '',
