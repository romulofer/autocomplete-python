...
export class JediDaemon {
  ...
  private reportNoInterpreter(detail?: string): void {
    ...
    this.options.notifier.warning(
      'autocomplete-python-pulsar could not find a Python interpreter.',
      {
        description:
          'Run **Autocomplete Python Pulsar: Select Interpreter**, or set `Python Executable Paths` in the package settings.',
        detail: detail ?? '',
        dismissable: true,
  ...
  private reportFatalError(error: DaemonFatalError): void {
    ...
    const descriptions: Record<DaemonFatalError['error'], string> = {
      ...
      'python-too-old':
        'autocomplete-python-pulsar needs Python 3.10 or newer. Choose a newer interpreter with **Autocomplete Python Pulsar: Select Interpreter**.',
